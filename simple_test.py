...
model = load_model("converted_keras (16)/keras_model.h5", compile=False, custom_objects=custom_objects)

# Load the labels
class_names = open("converted_keras (16)/labels.txt", "r").readlines()

# CAMERA can be 0 or 1 based on default camera of your computer
camera = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # Added CAP_DSHOW for Windows compatibility
...
while True:
    ...
    # Predicts the model
    prediction = model.predict(image, verbose=0)  # Added verbose=0 to reduce output noise
